//Given a ray described by an initial point P0 and a direction V both in world coordinates, check to see 
...
//mvMatrix is a matrix describing how to transform "vertices" into world coordinates which you will have to
//do to get the correct intersection in world coordinates.
...
function rayIntersectPolygon(P0, V, vertices, mvMatrix) {
	...
	//you should return both an intersection point and a parameter t.  The parameter t will be used to sort
	//intersections in order of occurrence to figure out which one happened first

}


function addImageSourcesFunctions(scene) { //Setup all of the functions that operate directly on the scene
    

	//Purpose: A recursive function provided which helps to compute intersections of rays with all faces in
	//the scene, taking into consideration the scene graph structure
   
	//Inputs: P0 (vec3): Ray starting point; V (vec3): ray direction; node (object): node in scene tree to process; 
	//mvMatrix (mat4): Matrix to put geometry in this node into world coordinates
	//excludeFace: Pointer to face object to be excluded (don't intersect with the face that this point lies on)
	//Returns: null if no intersection, {tmin:minimum t along ray, PMin(vec3): corresponding point, faceMin:Pointer to mesh face hit first}
	...
	var tmin = Infinity; //The parameter along the ray of the nearest intersection
		...
		//Recursively check the meshes of the children to make sure the ray doesn't intersect any of them first
			for (var i = 0; i < node.children.length; i++) {
				...
				//Multiply on the right by the next transformation of the childnode
